  ...
  if( isset( $_POST["com_btn"] ) ) { // echo "im in";
    ...
    if( $connObj ) { // echo "all cool"; 
      ...
      if( !empty( $comment ) && preg_match("/^[a-z]/i", $comment ) ) { // when comment is provided 
        $usr_id = $_SESSION["student_id"]; $usr_fname = $_SESSION["firstname"];
        ...
        if( mysqli_query( $connObj, $query ) ) {
          ...
          header( "location:./pro_det_stu.php?project_id=$pro_id" );
        }
        else {
          ...
          header("location:./pro_det_stu.php?project_id=$pro_id"); // send user to main page of projects.
        }
      }
      else {
